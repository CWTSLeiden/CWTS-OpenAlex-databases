...
select citation_id, citing_work_id, reference_seq, cited_work_id
into #citation
from
(
	select
		citation_id = row_number() over (order by work_id, reference_seq),
		citing_work_id = work_id,
		reference_seq,
		cited_work_id,
		[filter] = row_number() over (partition by work_id, cited_work_id order by reference_seq asc)
	from work_reference
	where work_id <> cited_work_id
) as a
where a.[filter] = 1
